...
usage() {
    ...
    echo "  version   Semver string, e.g. 0.4.0"
...
update_file \
    ...
    "\"version\": \"$VERSION\"" \
    "interagency-marketplace/marketplace.json (tool-time entry)"
...
git push
echo -e "${GREEN}Pushed tool-time${NC}"

cd "$MARKETPLACE_ROOT"
git add .claude-plugin/marketplace.json
git commit -m "chore: bump tool-time to v$VERSION"
git push
echo -e "${GREEN}Pushed interagency-marketplace${NC}"

echo ""
echo -e "${GREEN}Done!${NC} tool-time v$VERSION"
echo ""
